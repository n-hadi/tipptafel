import Adyen
import uuid
#from config.settings import TEST_API_KEY
...
def test_sessions_success():
    ady = Adyen.Adyen()
    client = ady.client
    #client.xapikey = TEST_API_KEY
    client.platform = "test"
    client.app_name = "Tipptafel"
    request = {}
    request['amount'] = {"value": "1000", "currency": "EUR"}
    request['reference'] = "id_abc849849934"
    request['merchantAccount'] = "Tipptafel485ECOM"
    request['returnUrl'] = "localhost:8000/@/skipper/"
...
'''
views
adyen = test_sessions_success().__dict__['message']
  return render(request,'payments/blocks/checkout.html', context={'sessionData':adyen['sessionData'], 'id': adyen['id'] })'''


'''
   <!-- Embed the Adyen Web script element above any other JavaScript in your checkout page. -->
 <script id="adyen-script" src="https://checkoutshopper-live.adyen.com/checkoutshopper/sdk/5.30.0/adyen.js"
    integrity="sha384-fTOXYkqjvevHJzN9rBXO8r1XzFtvP1F7LpPd8pJi5UGBH9H4XRmSoT6LHi0AicDB" crossorigin="anonymous"></script>
 <link rel="stylesheet" href="https://checkoutshopper-live.adyen.com/checkoutshopper/sdk/5.30.0/adyen.css"
    integrity="sha384-wNJf9kOfnzg6TcZUN6ysrsTbnTJtuwoDIgAWPr/uRy83KzYIQaf1zzr3MsbCdeUE" crossorigin="anonymous">
 <script>
    async function f(){
     const translations = {
          ...
          "de-DE": {
             "payButton": "Bezahlen",
             "payButton.redirecting": "Sie werden weitergeleitet...",
             ...
             "creditCard.holderName.placeholder": "Max Mustermann",
             "creditCard.holderName.invalid": "Ungültiger Karteninhabername",
             "creditCard.numberField.title": "Kartennummer",
             "creditCard.numberField.placeholder": "XXXX XXXX XXXX XXXX",
             ...
             "firstName": "Vorname",
             "firstName.invalid": "Vorname ist ungültig",
             "infix": "Vorwahl",
             "lastName": "Nachname",
             "lastName.invalid": "Nachname ist ungültig",
             ...
             "error.va.sf-cc-num.01": "Kartennummer ist ungültig",
             "error.va.sf-cc-num.02": "Das Feld für die Kartennummer ist leer",
             "error.va.sf-cc-num.03": "Nicht unterstützte Kartennummer eingegeben",
             "error.va.sf-cc-num.04": "Die Kartennummer hat die falsche Länge",
             "error.va.sf-cc-dat.01": "Ablaufdatum: Karte zu alt",
             "error.va.sf-cc-dat.02": "Ablaufdatum: Datum zu weit in der Zukunft",
             "error.va.sf-cc-dat.03": "Ablaufdatum: Ihre Karte läuft vor dem Abreisedatum ab",
             "error.va.sf-cc-dat.04": "Das Feld für das Ablaufdatum ist leer",
             "error.va.sf-cc-dat.05": "Das Ablaufdatum hat die falsche Länge",
             "error.va.sf-cc-mth.01": "Das Feld für den Ablaufmonat ist leer",
             "error.va.sf-cc-yr.01": "Das Feld für das Ablaufjahr ist leer",
             "error.va.sf-cc-yr.02": "Das Ablaufjahr hat die falsche Länge",
             "error.va.sf-cc-cvc.01": "Das Feld für den Sicherheitscode ist leer",
             "error.va.sf-cc-cvc.02": "Der Sicherheitscode hat die falsche Länge",
             ...
             "creditCard.encryptedCardNumber.aria.iframeTitle": "Iframe für Nummer der gesicherten Karte",
             "creditCard.encryptedCardNumber.aria.label": "Kartennummer",
             "creditCard.encryptedExpiryDate.aria.iframeTitle": "Iframe für Ablaufdatum der gesicherten Karte",
             "creditCard.encryptedExpiryDate.aria.label": "Ablaufdatum",
             "creditCard.encryptedExpiryMonth.aria.iframeTitle": "Iframe für Monat des Ablaufdatums der gesicherten Karte",
             "creditCard.encryptedExpiryMonth.aria.label": "Monat des Ablaufdatums",
             "creditCard.encryptedExpiryYear.aria.iframeTitle": "Iframe für Jahr des Ablaufdatums der gesicherten Karte",
             "creditCard.encryptedExpiryYear.aria.label": "Jahr des Ablaufdatums",
             "creditCard.encryptedSecurityCode.aria.iframeTitle": "Iframe für Sicherheitscode der gesicherten Karte",
             "creditCard.encryptedSecurityCode.aria.label": "Sicherheitscode",
             "creditCard.encryptedPassword.aria.iframeTitle": "Iframe für gesichertes Kartenpasswort",
             "creditCard.encryptedPassword.aria.label": "Die ersten zwei Ziffern des Kartenpassworts",
             "giftcard.encryptedCardNumber.aria.iframeTitle": "Iframe für Geschenkkartennummer der gesicherten Karte",
             "giftcard.encryptedCardNumber.aria.label": "Geschenkgutscheinnummer",
             "giftcard.encryptedSecurityCode.aria.iframeTitle": "Iframe für Sicherheitscode der gesicherten Geschenkkarte",
             ...
             "ach.encryptedBankAccountNumber.aria.iframeTitle": "Iframe für gesicherte Bankkontonummer",
             "ach.encryptedBankAccountNumber.aria.label": "Bankkonto",
             "ach.encryptedBankLocationId.aria.iframeTitle": "Iframe für gesicherte BLZ",
             ...
             "ctp.errors.ACCT_INACCESSIBLE": "Dieses Konto ist derzeit nicht verfügbar; es kann z. B. gesperrt sein.",
     ...
     const configuration = {
      ...
      clientKey: 'test_B4U4DJ5KJBCPLHYPW244GQHFKMNYI4ET', // Public key used for client-side authentication: https://docs.adyen.com/development-resources/client-side-authentication
      analytics: {
    ...
    var adyen_js = document.querySelector('#adyen-script');
      adyen_js.addEventListener('load', function () {
         f();
      })
      function translate_plugin(){
         console.log('hi')
         var pm_names = document.getElementsByClassName('adyen-checkout__payment-method__name');
         console.log(pm_names.length)
         for (let i = 0; i < pm_names.length;i++){
            console.log(pm_names[i].innerHTML)
            if (pm_names[i].innerHTML == 'Credit Card')
               pm_names[i].innerHTML = 'Kreditkarte'
            if (pm_names[i].innerHTML == 'Online bank transfer.')
               pm_names[i].innerHTML = 'Sofort'
         }
      }
